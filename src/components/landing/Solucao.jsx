    ...
    <section className="px-6 md:px-8 lg:px-0 relative overflow-hidden -mt-0 md:-mt-25 pt-8 md:pt-0 pb-8 md:pb-0">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-6 md:mb-20">
          <h2 className="text-orange-500 font-bold tracking-widest uppercase text-xs md:text-sm mb-3 md:mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 lg:gap-8">
          {solutions.map((item, index) => (
            <div 
              key={index} 
              className="group p-5 md:p-8 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] hover:border-orange-500/30 transition-all duration-300 backdrop-blur-sm"
            >
              <div className=" inline-block p-2 md:p-3 rounded-lg bg-orange-500/10 group-hover:scale-110 transition-transform">
                {item.icon}
              </div>
              <div className="text-[8px] md:text-[10px] font-bold text-orange-400 tracking-tighter uppercase mb-2">
                {item.tag}
              </div>
              <h4 className="text-base md:text-xl font-bold text-white mb-2 md:mb-4 group-hover:text-orange-500 transition-colors">
                {item.title}
              </h4>
